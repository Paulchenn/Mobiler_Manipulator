...
def visibilityPRMVisualize(planner, solution, actions, ax = None, nodeSize = 300, plot_only_solution=True, plot_robot=True):
    ...
    # Erstelle ein neues Dict OHNE 'lightblue'
    # Logik: Behalte (k, v), wenn v NICHT 'lightblue' ist
    filtered_color = {k: v for k, v in color.items() if v != 'lightblue'}

    # Wenn du die Listen separat brauchst:
    ...
    if not plot_only_solution:
        nx.draw_networkx_nodes(graph, pos_xy, ax = ax, nodelist=list(final_keys), node_color=list(final_values), node_size=nodeSize, alpha=0.5)
