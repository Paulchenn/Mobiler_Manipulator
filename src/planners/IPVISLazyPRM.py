...
def lazyPRMVisualize(planner, solution = [] , ax=None, nodeSize = 300):
    ...
    # draw graph
    nx.draw_networkx_nodes(graph, pos_xy, ax=ax, nodelist=list(color.keys()), node_color=list(color.values()), node_size=nodeSize)
    ...
    nx.draw_networkx_edges(G0, pos_xy, edge_color='b', width=3.0, style='dashed', alpha=0.5,)
    if collEdges != []:
        collGraph = nx.Graph()
        ...
        nx.draw_networkx_edges(collGraph, pos_xy, alpha=0.2, edge_color='r', width=5)
    ...
    if nonCollEdges != []:
        ...
        nx.draw_networkx_edges(nonCollGraph, pos_xy, alpha=0.8, edge_color='yellow', width=5)
    


    # draw start and goal
    if "start" in graph.nodes(): 
        nx.draw_networkx_nodes(graph, pos_xy, nodelist=["start"], node_size=300, node_color='#00dd00',  ax=ax)
        nx.draw_networkx_labels(graph, pos_xy, labels={"start": "S"},  ax=ax)


    if "goal" in graph.nodes():
        nx.draw_networkx_nodes(graph, pos_xy, nodelist=["goal"], node_size=300, node_color='#DD0000',  ax=ax)
    ...
    if solution != []:
        ...
        nx.draw_networkx_edges(Gsp, pos_xy, alpha=0.8, edge_color='g', width=10,  ax=ax)
