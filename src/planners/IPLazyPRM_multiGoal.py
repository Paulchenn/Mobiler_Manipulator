...
class LazyPRM(PRMBase):
    ...
    def planPath(self, startList, goalList, config):
        ...
        goal_ids = []
        for i, goal_pos in enumerate(checkedGoalList):
            goal_id = "goal_{}".format(i)
            self.graph.add_node(goal_id, pos=goal_pos)
        ...
        while maxTry < config["maxIterations"]: 
            ...
            try:
                ...
                # Iteriere über alle Ziele nacheinander
                for next_goal_id in goal_ids:
                    # Suche Weg vom aktuellen Start zum nächsten Ziel
                    segment = nx.shortest_path(self.graph, current_start_node, next_goal_id)
                    
                    # Segment anfügen (Achtung: Das erste Element ist doppelt, daher [1:] beim Anfügen, außer beim allerersten)
                    if not full_path:
                        full_path.extend(segment)
                    else:
                        full_path.extend(segment[1:])
                        
                    current_start_node = next_goal_id # Das Ziel ist der Start für die nächste Runde

                path = full_path # Das ist jetzt der komplette Weg über alle Punkte

            except nx.NetworkXNoPath:
                # Wenn IRGENDEIN Segment nicht gefunden wird -> Roadmap erweitern
                self._buildRoadmap(config["updateRoadmapSize"], config["kNearest"])
                maxTry += 1
                continue
