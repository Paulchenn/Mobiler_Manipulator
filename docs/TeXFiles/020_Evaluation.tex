...
\begin{table}[htbp]
    ...
    \begin{tabular}{l l l p{5cm}}
        ...
        \midrule
        \multirow{4}{*}{LazyPRM} 
        & Initial Roadmap Size & 500 & Hohe initiale Abdeckung (\enquote{Flooding}) \\
...
\begin{listNorm}
    ...
    \item \textbf{Komplexe Umgebungen:} Im Szenario \textit{Forest} (viele Hindernisse) zeigt \ac{VisibilityPRM} seine Stärke mit 100\,\% Erfolg. \ac{LazyPRM} bricht hier ein und findet nur in 50\,\% der Fälle eine Lösung, da der Arm oft in \enquote{Sackgassen} zwischen den Hindernissen gerät (siehe Abb.~\ref{fig:forest_bench_multi} und Anhang~\ref{sec:erg_multiRun_SelfCheck}  Abb.~\ref{fig:shelf_reach_bench_multi}).
\end{listNorm}
...
\begin{itemize}
    \item \textbf{Steigerung bei LazyPRM:} Die Erfolgsrate des \ac{LazyPRM} stieg von 50\,\% (mit Self-Check, siehe Abb.~\ref{fig:forest_bench_multi}) auf 100\,\% (ohne Self-Check, siehe Anhang Abb.~\ref{fig:forest_bench_multi_noCheck}).
...
Hingegen zeigt sich in den Engstellen-Szenarien (\textit{The Wall}, \textit{Narrow Passage}), dass die Deaktivierung der Eigenkollision keinen positiven Einfluss auf die Erfolgsrate des \ac{LazyPRM} hat (verbleibt bei ca. 0\,\%, vgl. Abb.~\ref{fig:narrow_passage_bench_multi} mit Abb.~\ref{fig:narrow_passage_bench_multi_noCheck}).
\begin{itemize}
